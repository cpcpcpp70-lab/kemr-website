      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-[90px]">
          ...
          <Link href="/" className="flex items-center gap-3">
            ...
            />
            <span className="text-white font-bold text-[27px] tracking-tight whitespace-nowrap">
              전기창업경영연구원
            </span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-0">
            {menuItems.map((item) => (
              ...
              >
                <button className="flex items-center gap-1 px-4 py-2 text-white/90 hover:text-gold text-[22px] font-medium transition-colors whitespace-nowrap h-[90px]">
                  {item.label}
                  <svg
        ...
        <div className="px-4 py-2">
          {menuItems.map((item) => (
            <div key={item.id} className="border-b border-white/10">
              <button
                className="flex items-center justify-between w-full py-3 text-white/90 text-[22px] font-medium"
                onClick={() =>
                  setOpenMobileMenu(
                ...
                <div className="pl-4 pb-2">
                  {item.submenu.map((sub) => (
                    <Link
                      key={sub.href}
                      href={sub.href}
                      className="block py-2 text-white/70 hover:text-gold text-[19px]"
                    >
                      {sub.label}
                    </Link>
                  ))}
