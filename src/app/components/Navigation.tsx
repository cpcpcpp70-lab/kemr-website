        ...
        <div className="flex items-center justify-between h-[90px]">
          ...
          <nav className="hidden lg:flex items-center">
            {menuItems.map((item) => (
              ...
              >
                <button className={`flex items-center gap-1 px-4 text-[20px] font-medium transition-colors whitespace-nowrap h-[90px] ${menuTextColor}`}>
                  {item.label}
                  <svg
                    className={`w-4 h-4 transition-transform duration-200 ${hoveredMenu === item.id ? "rotate-180" : ""}`}
                ...
                {/* 드롭다운 */}
                <div className={`absolute top-full left-0 min-w-[180px] bg-white shadow-2xl border-t-2 border-gold transition-all duration-200 ${
                  hoveredMenu === item.id ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"
                }`}>
                  {item.submenu.map((sub) => (
                    <Link
                      key={sub.href}
                      href={sub.href}
                      className={`block px-5 py-3 text-[17px] font-medium border-b border-gray-100 last:border-0 transition-colors whitespace-nowrap ${
                        pathname === sub.href
        ...
        <div className="px-4 py-2">
          {menuItems.map((item) => (
            <div key={item.id} className={`border-b ${isTransparent ? "border-white/10" : "border-gray-100"}`}>
              <button
                className={`flex items-center justify-between w-full py-3 text-[20px] font-medium ${isTransparent ? "text-white/90" : "text-gray-700"}`}
                onClick={() => setOpenMobileMenu(openMobileMenu === item.id ? null : item.id)}
              >
                {item.label}
                <svg className={`w-5 h-5 transition-transform ${openMobileMenu === item.id ? "rotate-180" : ""}`}
                ...
                <div className="pl-4 pb-2">
                  {item.submenu.map((sub) => (
                    <Link
                      key={sub.href}
                      href={sub.href}
                      className={`block py-2 hover:text-gold text-[17px] ${isTransparent ? "text-white/70" : "text-gray-500"}`}
                    >
                      {sub.label}
                    </Link>
                  ))}
