      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-[70px]">
          ...
          <Link href="/" className="flex items-center gap-3">
            ...
            />
            <span className="text-white font-bold text-[18px] tracking-tight whitespace-nowrap">
              전기창업경영연구원
            </span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-0">
            {menuItems.map((item) => (
              ...
              >
                <button className="flex items-center gap-1 px-4 py-2 text-white/90 hover:text-gold text-[15px] font-medium transition-colors whitespace-nowrap h-[70px]">
                  {item.label}
                  <svg
                ...
                >
                  {item.submenu.map((sub) => (
                    <Link
                      key={sub.href}
                      href={sub.href}
                      className={`block px-5 py-3 text-[13px] font-medium border-b border-gray-100 last:border-0 transition-colors whitespace-nowrap ${
                        pathname === sub.href
            ...
            <Link
              href="/consult"
              className="ml-4 px-5 py-2.5 bg-gold hover:bg-gold-light text-white text-[14px] font-bold rounded transition-colors whitespace-nowrap"
            >
              무료 상담신청
            </Link>
          </nav>
        ...
        <div className="px-4 py-2">
          {menuItems.map((item) => (
            <div key={item.id} className="border-b border-white/10">
              <button
                className="flex items-center justify-between w-full py-3 text-white/90 text-[15px] font-medium"
                onClick={() =>
                  setOpenMobileMenu(
          ...
          <Link
            href="/consult"
            className="block my-3 py-3 text-center bg-gold text-white text-[15px] font-bold rounded"
          >
            무료 상담신청
          </Link>
        </div>
